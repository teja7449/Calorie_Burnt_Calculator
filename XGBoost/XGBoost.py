...
model = joblib.load('XGBoost/calories_predictor_model.pkl')
...
def predict_calories():
    try:
        ...
        if not all([age, height, weight, duration]):
        ...
        if calories_burned[0] < 0:
            calories_burned[0] = 0  # Set to zero if negative
...
# Gender input using radio buttons
tk.Label(root, text="Gender:").grid(row=0, column=0)
gender_var = tk.StringVar(value='male')  # Default value

tk.Radiobutton(root, text="Male", variable=gender_var, value='male').grid(row=0, column=1)
tk.Radiobutton(root, text="Female", variable=gender_var, value='female').grid(row=0, column=2)
...
# Duration input
tk.Label(root, text="Workout Duration (in minutes):").grid(row=5, column=0)
duration_entry = tk.Entry(root)
duration_entry.grid(row=5, column=1)

# Body Temperature options
tk.Label(root, text="Body Temperature:").grid(row=4, column=0)
...
body_temp_dropdown.grid(row=4, column=1)
...
# Heart Rate options
tk.Label(root, text="Average Heart Rate:").grid(row=6, column=0)
...
heart_rate_dropdown.grid(row=6, column=1)
